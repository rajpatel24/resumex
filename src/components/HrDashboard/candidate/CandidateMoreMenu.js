...
export default function CandidateMoreMenu(props) {
  ...
  const formik = useFormik({
    ...
    const data = {office_location: officeLocation.length > 0 ? officeLocation : props.office_location, is_active: checked};
    const headers = {
      'Authorization': `Token ${localStorage.getItem('authToken')}`,
    }

    axios.put(constants.HTTP_METHOD + constants.HTTP_URL + constants.HTTP_PORT + "/api/v1/office-locations/" + props.id + '/', data, {headers})
        .then(function (response) {
              if (response.status == 200) {
                enqueueSnackbar("Office Location updated successfully !!", {
                  anchorOrigin: {
                                  vertical: 'top',
                                  horizontal: 'right',
                                },
  ...
  const handleDeleteChange = event =>{
    setIsOpen(false);
    const apiInstance = axios.delete(constants.HTTP_METHOD + constants.HTTP_URL + constants.HTTP_PORT + '/api/v1/office-locations/' + props.id, {headers: {"Authorization" : `Token ${localStorage.getItem('authToken')}`}})
    .then(function (response) {
      if (response.status == 204) {
        enqueueSnackbar("Office Location deleted successfully !!", {
          anchorOrigin: {
                          vertical: 'top',
                          horizontal: 'right',
                        },
            ...
            <FormikProvider value={formik}>
                <Form autoComplete="off" onSubmit={handleSubmit}>
                    ...
                    <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                        ...
                        defaultValue={props.office_location}
