...
var defaultSize = { width: utils_1.THUMBNAIL_WIDTH, height: utils_1.THUMBNAIL_WIDTH };
function FileOrganizer(_a) {
    ...
    var fileOrganizerRef = react_1.useRef(null);
    var gridRef = react_1.useRef(null);
    react_1.useImperativeHandle(_gridRef, function () { return gridRef.current; });
    var _b = react_1.useState(0), columnCount = _b[0], setColumnCount = _b[1];
    var _c = react_1.useState(), editingId = _c[0], setEditingId = _c[1];
    var _d = react_1.useState(), draggingId = _d[0], setDraggingId = _d[1];
    ...
    var _e = react_1.useState(function () { return thumbnailSize || getSize(); }), size = _e[0], setSize = _e[1];
    ...
    var handleOnDragChange = react_1.useCallback(function (id) {
        onDragChange === null || onDragChange === void 0 ? void 0 : onDragChange(id);
        setDraggingId(id);
    }, [onDragChange]);
    var handleItemKeyDown = react_1.useCallback(function (event, index, _file, draggableRef) {
        var indexDiff = 0;
        switch (event.key) {
            case 'ArrowLeft':
                indexDiff = -1;
        ...
        if (!gridRef.current)
            return;
        var siblingAtLocation = utils_1.getSibling(draggableRef.current, indexDiff);
        // If no meta key was pressed, focus item in direction of keys.
        if (siblingAtLocation && !(event.metaKey || event.ctrlKey)) {
            var focusable_1 = siblingAtLocation.querySelector(utils_1.focusableElementDomString);
            if (focusable_1) {
                hasMoved = true;
        ...
        if (!hasMoved)
            return;
        var isVisible = utils_1.isScrolledIntoView(siblingAtLocation, fileOrganizerRef.current).isVisible;
        if (isVisible)
            return;
        // Use react-window scrollToItem api for virtualized items.
        var rowIndex = utils_1.getRowAndColumnIndex(index + indexDiff, columnCount).rowIndex;
    ...
    var renderItem = react_1.useCallback(function (file, index, style) {
        if (!file)
            return react_1.default.createElement(react_1.Fragment, { key: '__null' }, null);
        ...
        var draggableRef = react_1.createRef();
    ...
    var handleOnKeyDown = react_1.useCallback(function (event) {
    ...
    var customDragLayerTranslate = react_1.useCallback(function (_a) {
    ...
    var fileOrganizerClass = classnames_1.default('ui__base ui__fileOrganizer', className);
    var Wrapper = noProvider ? react_1.default.Fragment : DndMultiProvider_1.DndMultiProvider;
    return (react_1.default.createElement(Wrapper, null,
        react_1.default.createElement("div", tslib_1.__assign({}, divProps, { className: fileOrganizerClass, ref: fileOrganizerRef, onClick: handleOnClick, onKeyDown: handleOnKeyDown, style: style, role: "grid", tabIndex: 0 }),
            react_1.default.createElement(MemoAutoSizer_1.MemoAutoSizer, { ref: gridRef, files: files, padding: pad, size: size, renderItem: renderItem, onColumnCountChange: setColumnCount }),
            onRenderDragLayer ? (react_1.default.createElement(DragLayer_1.DragLayer, { customTranslate: customDragLayerTranslate },
                react_1.default.createElement("div", { className: "ui__fileOrganizer__draglayer", style: { height: utils_1.THUMBNAIL_WIDTH, width: utils_1.THUMBNAIL_WIDTH } }, onRenderDragLayer()))) : undefined)));
}
