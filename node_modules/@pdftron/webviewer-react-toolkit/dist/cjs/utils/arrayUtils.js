...
function separateItemsWithTarget(allItems, separateIds, targetId) {
    var _a = separateItemsById(allItems, separateIds), separated = _a[0], remaining = _a[1];
    var target = separated.find(function (item) { return item.id === targetId; });
...
function moveMultiFromIndexToIndex(prev, moveIds, fromIndex, toIndex) {
    var _a;
    var targetId = (_a = prev[fromIndex]) === null || _a === void 0 ? void 0 : _a.id;
    if (targetId === undefined || !moveIds.includes(targetId))
        return prev;
    var _b = separateItemsById(prev, moveIds), separated = _b[0], remaining = _b[1];
    var targetIndex = separated.findIndex(function (item) { return item.id === targetId; });
    ...
    if (insertionIndex > remaining.length)
        return tslib_1.__spreadArrays(remaining, separated);
    if (insertionIndex < 0)
        return tslib_1.__spreadArrays(separated, remaining);
    return tslib_1.__spreadArrays(remaining.slice(0, insertionIndex), separated, remaining.slice(insertionIndex));
}
